...
def avg_sentiment(selected_user, df):
    ...
    temp_df = df.copy()
    if selected_user != 'Overall':
        temp_df = temp_df[temp_df['Sender'] == selected_user]

    # Return neutral if no messages found
    if temp_df.empty:
        return 0.0, "Neutral"

    # Apply sentiment detection
    temp_df['Sentiment'] = temp_df['Message'].apply(detect_sentiment)

    # Calculate average sentiment
    average_sentiment = temp_df['Sentiment'].mean()
    ...
    return average_sentiment, sentiment_label
